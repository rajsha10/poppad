...
function Trade({ toggleTrade, token, provider, poppad }) {
  ...
  async function buyHandler(form) {
    ...
    const cost = await poppad.getCost(token.sold)
    const totalCost = cost * BigInt(amount)

    const signer = await provider.getSigner()

    const transaction = await poppad.connect(signer).buy(
      token.token,
  ...
  async function getSaleDetails() {
    const target = await poppad.TARGET()
    setTarget(target)

    const limit = await poppad.TOKEN_LIMIT()
    setLimit(limit)
